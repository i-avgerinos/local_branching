...
def solve_subproblem(nJobs, nMachines, processing_times, setup_times, resources, neighbourhood, big_M, best_ub):
    subproblem = CpoModel()
    ...
    # Objective function
    subproblem.add(subproblem.minimize(sum(subproblem.end_of(processJob[(s, i, m)])*selectedSolution[s] for s in range(len(neighbourhood)) for m in range(nMachines) for i in range(len(neighbourhood[s][m]))))) # Objective function: minimisation of total completion times
...
def relaxation(solution, processing_times, setup_times):
